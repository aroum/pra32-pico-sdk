...
static uint8_t s_program_table_panel_parameters[] = {
  ...
  SEQ_VELO_7     ,
    ...
    SEQ_MODE,       SEQ_ACT_STEPS,   SEQ_TRANSPOSE,   SEQ_STEP_NOTE,
...
template <boolean BYPASS_FX = false> class PRA32_U_Synth {
  PRA32_U_Osc m_osc;
  ...
  using ChorusFx =
      std::conditional_t<BYPASS_FX, std::monostate, PRA32_U_ChorusFx>;
  using DelayFx =
      std::conditional_t<BYPASS_FX, std::monostate, PRA32_U_DelayFx>;
...
public:
  PRA32_U_Synth()
      ...
      : m_osc(), m_filter(), m_amp(), m_noise_gen(), m_lfo(), m_eg(),
        m_chorus_fx(), m_delay_fx()
        ...
        ,
        m_program_number_to_write(0), m_wr_prog_to_flash_cc_value(0),
  ...
  INLINE void initialize() {
    ...
    for (uint32_t program_number = (PRESET_PROGRAM_NUMBER_MAX + 1);
         program_number <= PROGRAM_NUMBER_MAX; ++program_number) {
      if ((EEPROM.read(program_number * 128) == 'U') &&
          (EEPROM.read(program_number * 128 + 1) == program_number)) {
        for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
          ...
          uint8_t value_read =
              EEPROM.read(program_number * 128 + control_number);
          if (value_read < 128) {
  ...
  /* INLINE */ void control_change(uint8_t control_number,
  ...
  /* INLINE */ void
  write_parameters_to_program(uint8_t program_number_to_write) {
    ...
    if (program_number_to_write <= PROGRAM_NUMBER_MAX) {
      for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
           ...
           ++i) {
        uint32_t control_number = s_program_table_parameters[i];
        EEPROM.write(program_number_to_write * 128 + control_number,
                     m_current_controller_value_table[control_number]);
      }
  ...
  INLINE int16_t process(int16_t mono_input, int16_t &right_level) {
    ...
    // voice_mixer_output_clamped = clamp((voice_mixer_output << 1) +
    // mono_input, (-INT16_MAX), (+INT16_MAX))
    volatile int32_t voice_mixer_output_clamped =
        (voice_mixer_output << 1) + mono_input - (+INT16_MAX);
    voice_mixer_output_clamped =
        (voice_mixer_output_clamped < 0) * voice_mixer_output_clamped +
        (+INT16_MAX) - (-INT16_MAX);
    voice_mixer_output_clamped =
        (voice_mixer_output_clamped > 0) * voice_mixer_output_clamped +
        (-INT16_MAX);
    ...
    if constexpr (BYPASS_FX == false) {
      chorus_fx_output_l =
          m_chorus_fx.process(voice_mixer_output, chorus_fx_output_r);

      delay_fx_output_l = m_delay_fx.process(
    ...
    right_level =
        delay_fx_output_r + (prev_output_error_r > s_output_error_r) * 22;
...
private:
  ...
  INLINE void set_voice_mode(uint8_t controller_value) {
    ...
    static uint8_t voice_mode_table[6] = {
        VOICE_POLYPHONIC, VOICE_PARAPHONIC,   VOICE_MONOPHONIC,
...
#if 1
    } else if (m_lfo_osc_dst == 1 ||
               m_lfo_osc_dst >= 32) { /* OSC_DST_PITCH_2 */
#else
  ...
  INLINE void update_eg_and_amp_eg() {
    ...
    if (m_controller_value_eg_amp_mod == 1 ||
        m_controller_value_eg_amp_mod >= 64) {
#else
    if (m_controller_value_eg_amp_mod >= 64) {
      ...
      if (m_controller_value_rel_eq_decay == 1 ||
          m_controller_value_rel_eq_decay >= 64) {
#else
